          ...
          <div className="bg-brand-dark rounded-[2.5rem] p-8 text-white space-y-8 shadow-2xl shadow-black/20">
            ...
            {paymentMethod === 'paypal' && (
              <div className="space-y-4 animate-in slide-in-from-top-4">
                {isOutOfRange ? (
                  <div className="p-6 bg-rose-50 border border-rose-100 rounded-[2rem] text-center space-y-2">
                    <AlertCircle className="text-rose-500 mx-auto" size={32} />
                    <p className="text-rose-700 font-black uppercase text-xs tracking-widest">Pago deshabilitado</p>
                    <p className="text-rose-500 text-[10px] font-medium">Lo sentimos, no podemos procesar pagos de PayPal para direcciones fuera de nuestro radio de entrega de 8km.</p>
                  </div>
                ) : (
                  <>
                    <PayPalButtons 
            ...
            {isPaid ? (
              ...
              <button
                ...
                disabled={isProcessing || isOutOfRange || !paymentMethod || (paymentMethod === 'cash' && (!cashAmount || parseFloat(cashAmount) < total))}
                className={`w-full py-5 text-white rounded-2xl font-black text-sm uppercase tracking-widest transition-all flex items-center justify-center gap-3 shadow-xl ${isOutOfRange ? 'bg-rose-500 hover:bg-rose-600 shadow-rose-500/20' : 'bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/20'} disabled:bg-slate-700 disabled:cursor-not-allowed`}
              >
                {isOutOfRange ? "FUERA DE COBERTURA" : isProcessing ? <Loader2 className="animate-spin" size={24} /> : (
