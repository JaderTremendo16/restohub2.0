...
import { GET_USERS, GET_ALL_LOYALTY_ACCOUNTS, GET_POINT_HISTORY, GET_LOYALTY_ACCOUNT } from '../../graphql/operations';
...
const ClientManagement = () => {
  ...
  const { data: userData, loading: loadingUsers } = useQuery(GET_USERS, {
    variables: { b: currentUser?.branch }
  });
  
  const { data: loyaltyData } = useQuery(GET_ALL_LOYALTY_ACCOUNTS);

  const users = (userData?.users || []).filter(u => u.role !== 'admin');
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Administra la base de datos de usuarios y su fidelización.</p>
        </div>
      </div>
        ...
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <span className="text-xs font-black text-brand-600 px-2 py-0.5 bg-brand-50 rounded-lg">CLIENTE PREMIUM</span>
            </div>
          </div>
