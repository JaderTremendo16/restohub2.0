...
import { Megaphone, Plus, Power, MapPin, Tag, Trash2 } from 'lucide-react';
...
const PromotionsManagement = () => {
  ...
  const { data, loading, refetch } = useQuery(GET_PROMOTIONS, {
    variables: { activeOnly: false, branch: currentUser?.branch }
  });
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Lanza ofertas temporales y descuentos por sede.</p>
        </div>
        <button 
      ...
      {loading ? (
        <div className="py-20 text-center text-slate-400">Cargando promociones...</div>
      ) : promos.length === 0 ? (
        <div className="py-20 text-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 text-slate-400">
           No hay promociones activas. ¡Crea una para atraer más clientes!
        </div>
      ) : (
          ...
          {promos.map((p) => (
            <div key={p.id} className={`group bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all relative overflow-hidden flex flex-col ${!p.isActive ? 'opacity-60 grayscale-[0.5]' : ''}`}>
              ...
              <div className="flex gap-2 pt-6 border-t border-slate-50 justify-between items-center">
                ...
                >
                  <Power size={14} /> {p.isActive ? 'Pausar' : 'Activar'}
                </button>
              </div>
            </div>
          ))}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  placeholder="Ej: Black Friday Burger"
                />
              </div>
              ...
              <div className="p-4 bg-brand-50 rounded-2xl border border-brand-100 flex gap-3 italic">
                 <MapPin className="text-brand-600 shrink-0" size={18} />
                 <p className="text-[10px] font-bold text-brand-600 leading-normal">Esta promoción se aplicará automáticamente a tu sede actual: <strong className="uppercase">{currentUser?.branch || 'General'}</strong></p>
              </div>
            </div>
