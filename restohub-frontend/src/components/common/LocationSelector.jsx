    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="p-8 text-center">
          ...
          <h3 className="text-2xl font-black text-slate-900 mb-2">¡Configurar tu Acceso!</h3>
          <p className="text-slate-500 mb-8">Selecciona tu país y sede administrativa para comenzar.</p>

          {(loadingCountries || loadingLocations) ? (
            <div className="py-12 flex flex-col items-center gap-4">
              <Loader2 className="animate-spin text-brand-600" size={32} />
              <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Sincronizando sedes...</p>
            </div>
          ) : (
            <>
              {step === 1 && (
