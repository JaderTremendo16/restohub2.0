...
import { GET_ALL_RATINGS } from '../../graphql/loyaltyOperations';
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium italic">Escucha las opiniones de los clientes de la sede: <span className="text-brand-600 font-bold uppercase tracking-tight px-2 py-0.5 bg-brand-50 rounded-lg">{branchName}</span></p>
        </div>
      </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan="5" className="px-8 py-20 text-center">
                    ...
                    <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Sincronizando reseñas...</p>
                  </td>
                </tr>
              ) : ratings.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-8 py-20 text-center text-slate-400 italic">
                    No hay reseñas registradas para esta sede aún.
                  </td>
                </tr>
              ) : (
                ratings.map((r) => (
